...
// biome-ignore-all lint/correctness/useExhaustiveDependencies: This port keeps scroll state in refs and mutable objects so event listeners stay stable.
...
export const useStickToBottom = (
	...
	const optionsRef = useRef<StickToBottomOptions>(options);
...
function useRefCallback<T extends (ref: HTMLElement | null) => unknown>(
	callback: T,
	deps: DependencyList,
) {
	const result = useCallback((ref: HTMLElement | null) => {
		result.current = ref;
		return callback(ref);
	}, deps) as unknown as MutableRefObject<HTMLElement | null> &
...
function mergeAnimations(...animations: (Animation | boolean | undefined)[]) {
	...
	let animation = animationCache.get(key);

	if (!animation) {
		animation = Object.freeze(result);
		animationCache.set(key, animation);
	}

	return instant ? "instant" : animation;
}

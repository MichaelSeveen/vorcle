...
import { formatHourLabel, groupEvents } from "../../config/utils";
...
export default function CalendarWeekView({
	...
	const groupedEventsByDay = useMemo(() => {
		return weekDays.reduce<Record<string, ReturnType<typeof groupEvents>>>(
			(acc, day) => {
				...
				acc[dayKey] = groupEvents(dayEvents);
			...
			>
				<div>
					<WeekViewMultiDayEventsRow
						selectedDate={weekDays[0] ?? selectedDate ?? new Date()}
						...
						<div className="grid flex-1 grid-cols-7 divide-x border-l">
							{weekDays.map((day, index) => (
								<motion.span
									...
									className="py-1 sm:py-2 text-center text-xs font-medium text-foreground"
				...
				<ScrollShadow className="h-[736px] overflow-auto">
					<div className="flex">
						{/* Hours column */}
						...
						>
							<div className="grid grid-cols-7 divide-x">
								{weekDays.map((day, dayIndex) => {
									const dayKey = format(day, "yyyy-MM-dd");
									const groupedEvents = groupedEventsByDay[dayKey] ?? [];

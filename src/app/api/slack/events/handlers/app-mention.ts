...
import type { AppMentionEvent, WebClient } from "@slack/web-api";

interface MentionsProp {
  event: AppMentionEvent;
  say: SayFn;
  client: WebClient;
}

export async function handleAppMention({ event, say, client }: MentionsProp) {
  try {
    const eventId = `app_mention-${event.channel}-${event.user}`;
    const eventTs = event.event_ts || event.ts;

    console.log("Event dey here", event);
